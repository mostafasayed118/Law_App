...
void main() {
  group('FakeBookingGateway.fetchSlots', () {
    test(
      'returns the fixed synthetic slot list, deterministic per call',
      () async {
  ...
  group('FakeBookingGateway.confirm', () {
    test(
      'persists in-memory and returns sequential synthetic reference ids',
      () async {
        final FakeBookingGateway gateway = FakeBookingGateway();

        final BookingConfirmation first = (await gateway.confirm(
          _draft(),
        )).valueOrNull!;
        final BookingConfirmation second = (await gateway.confirm(
          _draft(),
        )).valueOrNull!;
    ...
    test(
      'never persists request content — only the confirmation is stored',
      () async {
        final FakeBookingGateway gateway = FakeBookingGateway();
        ...
        expect(stored.toString(), isNot(contains('Sensitive matter details')));
      },
    );
  });
}

BookingRequest _draft() =>
    const BookingRequest(category: BookingCategory.general);
