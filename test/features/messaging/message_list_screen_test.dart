...
void main() {
  ...
  group(
    'message thread list surface (Phase 9 slice 9.1; Phase 12 slice 12.1)',
    () {
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          // D-MSG3 re-scoped by Phase 12 D-C2: rows still carry no chevron
          // (contrast the matter list, where every row carries the details
          // affordance), and the ONLY InkWell in the list lives inside a
          // resolved row's "View matter" chip — the reverse cross-link
          // affordance. Unresolved rows keep no tap target at all.
          ...
          expect(
            listInkWells.evaluate().length,
            chipInkWells.evaluate().length,
          );
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          expect(
            find.text('No message threads are available.'),
            findsOneWidget,
          );
