...
- Tests (1423 total): Result, AppError, UseCase, ViewState, AuthCubit (demo
  ...
  assertions.
- Coverage: **1423 tests** (2026-09-23 — the D-15 video-consultation demo
  slice: the synthetic `VideoGateway` seam + 4-session deterministic fake
  (zero real media, zero writes, posture doc
  `docs/video_scope_decision_2026-08-11.md`), the join/leave call surface
  with the live-badge/elapsed/mic-camera demo chrome, the home video entry
  card riding the booking gate, the discovery practice-area deep link
  (`/discovery?area=`), the booking-success join CTA, and the dead-tap
  wiring (notification bell, avatar, all four practice-area cards); prior
  2026-09-22 — the P1.7 state-honesty slice: a typed
  ...
  (gateway `markNotificationsRead`, fake mirror, Supabase RPC impl) + the
  feed's tappable unread rows (the D-C2 pin deliberately re-scoped, D-F6;
  shape+affordance unread marker, never color alone; read rows stay
  non-interactive) + the D-F6 l10n pin — on top of the AI research slice
  (plan
  ...
  and the slot-error retry fix (was wired to `continueFromCategory`, which
  no-ops off the category step — now `retryLoadSlots` re-runs the fetch
  from the date-time step), on top of the admin terminal-state arms: two
  ...
  `platform_admin_states`) on top of the booking wizard step pipeline split
  into `part` files under `lib/features/booking/presentation/`
  (`booking_category_step` /
  `booking_datetime_step` / `booking_review_step` / `booking_success_step` /
  `booking_selectable_tile`) — both pure mechanical splits, zero behavior
  ...
  trailing gap) consolidated into one local `_WorkspaceBlock` (superseded
  by C2), on top of
  ...
  item-builder). On top of the E9 extraction: shared
  `AppCenteredRetry` (the attorney-profile + matter-details centered error
  arms unified — message + retry callback + localized retry label passed
  in), partially addressing close-out residual #5 (centered-message
  screens; booking's start-aligned `ViewError` arm stays a separate shape).
  On top of the E8 extraction: the four matter
  workspace section rows (`_DocumentRow`/`_FileRow`/`_InvoiceRow`/
  `_ThreadRow`) now delegate to shared `AppTile` (title + `subtitles`, no
  icon, `onTap: null`), closing close-out residual #6 for the workspace
  rows; the thread-detail `_MessageTile` stays excluded as a genuinely
  different shape. On top of the E7 extraction: shared
  ...
  `showConfirmDialog` (the destructive member-removal / delete-account /
  demo-account-delete confirm dialogs unified, error-tinted confirm), on
  top of the E2 extraction: shared `AppTile`
  (the four search result tiles inlined + five list rows slimmed to thin
  wrappers, with the D-C2 nullable-onTap / D-MSG1 chevron-free contracts
  baked in), on top of the E3 extraction: shared `ViewStateSwitch`
  (the loading/empty/error/offline/unauthorized arms of 11 list screens and
  matter workspace sections de-duplicated into one switch), on top of the
  E1/E6 extraction: shared `AppEntryCard` (nine feature entry cards reduced
  to thin wrappers) + centralized `formatMediumDate`/`formatMediumDateTime`
  (12 call sites de-duplicated), on top of the theme pass: the persisted
  ...
  projection (+15 declarations)),
  building
  on 1093 (2026-08-09, partner org-audit slice — Task 3:
  `OrganizationGateway.readOrgAudit` on the org seam — `SupabaseOrgApi` RPC
  wrapper + guarded row→`AuditEntry` mapper + fake mirror (deterministic
  redacted demo trail, denied-never-empty, honest-empty fresh org),
  scope `docs/partner_org_audit_scope_2026-08-09.md`), building
  on 1084 (2026-08-09, the standalone billing-invoices
  surface — spec §6 row 158: `/invoices` route + `BillingInvoicesScreen` +
  home entry riding the `canViewDocuments` gate per D-BI5 + six widget tests
  incl. the D-BI1 metadata-only pins), building
  on 1078 (2026-08-09, partner org-audit slice — Task 2:
  ...
  mapping/matterRef-fallback/failure-mapping/impl/cubit tests), building on
  the audited `send_message` RPC (D-SM2): the seam's
  ...
  trails + org-scoped selector); `flutter analyze` and the
  format gate
...
`docs/current_applied_surface_2026-08-08.md` §1d). Current suite: **1426
tests passing** (1423 tracked declarations, ledger `verify_ledger.sh` PASS).
