...
> **DRAFT 2026-08-04** — prepared for owner ratification (the Phase 8/9
> governance-first flow: provenance → decision record → slices behind the
> standard slice gate). Implementation must not start until the decision
> record (D-W1…D-W6) is ratified and the roadmap gains the Phase 10 row.
...
- **Declared future work in three ratified scope notes:**
  ...
  - `docs/matter_dashboard_scope_2026-08-03.md` §3: no matter messaging /
    documents from any matter surface was a non-goal *only because* rows
    154/155 kept their own §11 gates — **Phases 8 and 9 shipped those
    surfaces**, so the per-matter cross-link is now unblocked.
- §13 boundary: `docs/features_roadmap_2026-08-03.md` §13 still defers the
...
| D-W2 | `Document` VO gains a **`matterRef`** field — one of the 5 known synthetic matter titles, the same shape `MessageThread.matterRef` already uses (D-MSG4). **No new identity surface**: the per-matter association is keyed on the synthetic matter title shared across the fakes (D-V4/D-M4/D-MSG4), never on ids, so the workspace cannot imply server-side scoping. The `Document` props-pin test is updated as a deliberate edit | **draft 2026-08-04** |
| D-W3 | Entry points: inline sections on the existing `/matters/:matterId` details screen (which already loads the synthetic matter list and resolves by id, D-M5). **No new routes**; the standalone vault and messages list surfaces keep their current behavior unchanged | **draft 2026-08-04** |
| D-W4 | **Body-less + metadata-only lines preserved on the matter surface**: no document preview/download, no thread-open affordance, no composer, no send/reply on the sections — the AC-2 pins of Phases 8/9 extend to the sections | **draft 2026-08-04** |
| D-W5 | Capability gating **reuses** `canViewDocuments` / `canViewMessages` (already true for every bootstrap role; navigation hints only, never authorization). A matter details screen renders a section only when the corresponding capability is granted | **draft 2026-08-04** |
| D-W6 | All new strings resolve in EN/AR/TR; per-matter sections carry the local-only demo framing (R1) | **draft 2026-08-04** |
...
- **R2 — body-less/read-only line:** the Phase 8/9 AC-2 absences hold on
  ...
  unexercised.
- **R3 — §13 boundary:** the per-matter view is client-side over synthetic
  lists; it must not imply server-side matter workspaces or scoping. The
  deferral text in roadmap §13 is preserved.
