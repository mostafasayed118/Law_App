...
> converts the open video-consultation row (**spec D-15 — video
...
- **Label collision — RESOLVED (2026-08-11, tracked_deviations D-T8):**
  the spec's decision table assigned **D-11 to video consultation** while
  the repo's `docs/d11_billing_payments_decision_2026-08-08.md` used the
  **D-11 label for billing/payments** (that record supersedes the spec's
  D-09 payment row) and `docs/p0_decision_capture.md` had no D-11 entry.
  The owner resolved it by **renumbering the video row D-11 → D-15**
  (the next free number after D-14) in the spec's four references — the
  decided billing record keeps D-11.
...
### B. Provider & data-handling (the spec D-15 row)

| # | Question | Options (default in **bold**) |
|---|---|---|
| B-1 | Provider for any future real integration? | **undecided — record one the way the billing record (D-11) did for Paymob when the slice is planned** / decide now (e.g. a hosted WebRTC/telehealth vendor) |
...
- [ ] B-1…B-4 answered → the provider/data-handling decision text (spec D-15 + D-04 dependency)
- [ ] C-1…C-3 answered → the slice plan shape
- [x] **Label collision — RESOLVED 2026-08-11** — the video row is renumbered to **D-15** (tracked_deviations D-T8); the billing record keeps D-11
...
- DRAFTED 2026-08-11 (question sheet only); no decision taken, no code,
  ...
  and the video scope note is written from those answers (the D-11 label
  collision is resolved — tracked_deviations D-T8).
