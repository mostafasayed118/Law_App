...
- **Status:** **STEP 1 SHIPPED 2026-08-09 + r1 REHEARSAL PASSED (2026-08-09,
  ...
  **D-45.1 configured-build verification EXECUTED 2026-08-09** on the dev
  project (evidence `docs/f01_client_swap_verification_evidence_2026-08-09.md`):
  the create flow's live `create_matter` call returned `4a8425d4-…`, the §8
  `matter:create`/`allowed` row appeared through the org-audit read path
  (`read_org_audit`, resource = the returned id, `redacted_summary` =
  `matter created`), the RLS read-back showed 5/5 matters (4 prior + the new
  one, assigned attorney = partner), and the owner-assignment negative
  returned the exact F2-D2 fragment (`platform owner cannot be assigned to a
  matter`) the client maps to `matter_write_owner_forbidden`. **The F-01
  chain (server + client + live verification) is CLOSED 2026-08-09.**
