...
- [ ] **3. Policy battery** — touches: `supabase/tests/07_storage_rls.sql`
  ...
  `files_storage_select` on storage.objects present) + forward pin narrowed
  to `('messages')`, **and the harness header + D-P0C1(b) forward-pin
  comments** ("all nine" → "all ten"; "individual messages/files still
  absent" → "individual message rows/bodies still absent; file storage
  shipped as the fourth un-deferral") — done when: static `--check` green
