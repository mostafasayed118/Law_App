...
| **Crash-class sweep + pins** | **complete** — the 4 `_boundRpc` bindings were the entire exposure | Full lib/ audit (`f2b5c30`): all 15 `.rpc(` call sites route through the 4 fixed bindings · no `count:`-requested rpc · no `.select()` on rpc builders · the 7 `from().select()` table seams resolve to **raw row lists** by the same contract and were drift-pinned with a sibling suite. Total **5 contract pins** against a real `SupabaseClient` + mocked HTTP — 3 RPC (`supabase_rpc_await_contract_test.dart`) + 2 table-select (`supabase_table_select_await_contract_test.dart`, plain + chained `.eq()`) — so any future postgrest await-shape change fails in CI, not on a device. Suite 1306 → 1308 runtime / 1303 → 1305 tracked. |
...
| 12 | RPC-binding fix `f1308cd` (08-11) | **client-side** postgrest await-contract fix in the 4 `*ApiImpl._boundRpc` bindings (the create-org on-device crash; root cause + verification in §1d) + 3-test RPC regression-pin suite + verification `d26aef7` | **Server surface unchanged** — 13 tables / 12 public + 1 storage / 20 RPC-EXECUTE (the fix corrected how the client awaits RPC responses, not what the server exposes) |
| 13 | crash-class sweep `f2b5c30` (08-11) | **client-side** — full lib/ sweep confirmed the 4 bindings were the entire exposure; +2 table-select await-contract pins (raw row list, plain + chained `.eq()`), total **5 pins** (3 RPC + 2 table-select) | **Server surface unchanged** — 13 tables / 12 public + 1 storage / 20 RPC-EXECUTE (pins only, no server touch) |
