...
- §14 boundary: `docs/features_roadmap_2026-08-03.md` §14 defers
  **matters** (and documents/messages/storage/realtime/audit/billing/AI)
  ...
  (table + RLS + storage + realtime) stays §14-deferred and untouched.
...
| D-M3 | **§14 boundary**: this phase is client-only demo surface. The real matters data path (table, RLS, storage, realtime) stays deferred per roadmap §14; nothing here grants or implies server-side matter access ("an org role alone never grants matter access" is untouched) | **ratified 2026-08-03** |
...
- **R3 — §14 boundary:** the client-only surface must not imply server
  matter access; the deferral text in roadmap §14 is preserved, and the
