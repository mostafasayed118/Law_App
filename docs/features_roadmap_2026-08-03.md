...
> (`69622ce`, suite 705/README 702, pushed to `origin/main`);
...
> no-silent-EN-copy asserts; suite 706/README 703; pushed to
> `origin/main`), closing Phase 12
...
(D-C4); suite 705/README 702, ledger PASS 115 — pushed to
`origin/main`. Scope note `docs/reverse_crosslink_scope_2026-08-04.md`
...
> **§14 PLAN CLOSE (2026-08-08) — nine of ten un-deferred and SHIPPED.**
> The consolidated close note (`docs/p14_plan_complete_2026-08-08.md`)
...
> T1–T8. **The FIRST NEW surface shipped end-to-end 2026-08-11: the
> notification feed** — owner-authorized (D-N1, not in the spec), it ran
> the same T1–T8 pipeline to a live close: scope DECIDED
> (`docs/notification_feed_scope_2026-08-11.md`) → T1 review → artifacts
> (`14_notifications` + `.down` + `notifications_select_org`) → battery 14
> + harness re-scope (13 tables / 13 RLS / 12 public policies) → r1
> 86/0/0 ×2 → dated apply-approval + dev apply → matrix §4 +
> applied-surface §1b addenda → T8 env-gated `NotificationGateway` swap
> (suite 1303) → configured-build E2E walkthrough evidence
> (`docs/notification_feed_e2e_walkthrough_evidence_2026-08-11.md`).
> Billing planning is
> un-blocked (D-11, planning only) by the Paymob spec
> `docs/paymob_integration_spec_2026-08-11.md`; the conflict/waiver/wall/
...
> with the billing record; tracked_deviations D-T8) and ratified
> demo-posture decisions (`docs/conflict_scope_decision_2026-08-11.md`,
> `docs/video_scope_decision_2026-08-11.md`). The remaining planning
> artifacts are planning only — none approved, none built.
...
storage, realtime — read + live delivery — audit surfacing, the audited
send path, and billing invoices read now un-deferred — the **ninth**
un-deferral — see the flip notes below; **AI is the only path left**).
...
1044, ledger PASS 115). **Billing is the ninth per-feature un-deferral,
SHIPPED 2026-08-08** — the D-11 decision (Paymob for any real
...
scope, D-04 residency confirmed; `docs/d11_billing_payments_decision_2026-08-08.md`)
un-blocked a billing-invoices read-metadata slice that shipped with the
full discipline (RLS-gate review → artifacts → battery → rehearsal r1
genuinely executed 78/0/0 → dated apply approved + executed (tables/RLS
11→12, public policies 10→11, 4 demo invoices) → matrix §4 addendum →
