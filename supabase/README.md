...
| `tests/14_notification_rls.sql` | **Notification-feed read battery** (`docs/notification_feed_gate_review_2026-08-11.md` Q2/Q3/Q6, D-N1..D-N7): `notifications_select_org` positives (partner-a 6 / client-a 6 — the no-role-hierarchy pin, RE-PINNED D-P6: +2 producer rows from battery 10's committed sends / partner-b 1 — org scoping by count) + cross-org / `platform_owner_admin` deny-always / suspended / anon denies + the `category` CHECK (D-N4 mapping contract) + org-delete cascade + the structural column-inventory pin (Q1 — no user-identity/content/raw-text columns) |
| `tests/15_notification_producer_rls.sql` | **Notification producer battery** (`docs/notification_feed_producer_gate_review_2026-08-11.md` Q1-Q6, D-P1..D-P6): the audit-mirror trigger — delta-based `matter:create`/`message:create` map positives (in-transaction, rolled back) with gate visibility through `notifications_select_org` + fixed-summary redaction pins (title/body never leak), the outcome / unmapped-action / NULL-org filters, the EXECUTE-deny (D-P4), and in-txn atomicity residue checks (D-P6) |
